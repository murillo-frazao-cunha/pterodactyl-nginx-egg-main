...
VERSION_FILE="/etc/enderd/VERSION"
API_BASE_URL="https://api.tavuru.de"
REPO_OWNER="Ym0T"
REPO_NAME="pterodactyl-nginx-egg"
CONTAINER_ROOT="/etc/enderd"
TEMP_DIR="/etc/enderd/tmp/autoupdate"
...
apply_update() {
  ...
  if [[ $updated_files -gt 0 ]]; then
    ...
    if [[ "$self_update_required" == "true" ]]; then
      ...
      echo -e "${CYAN}[AutoUpdate] Staged location: /etc/enderd/.autoupdate_staged${NC}"
    fi
